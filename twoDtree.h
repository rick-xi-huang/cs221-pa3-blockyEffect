...
private:
   ...
   */
   ...
   void renderPixel(const Node * subRoot, PNG & im);

   Node * copyTree(const Node * other);

   bool checkLeaves(RGBAPixel average, const Node * subRoot, int tol);

   void clearNode(Node *& subRoot, int tol);

   int countLeaves(const Node * subRoot, int tol);
